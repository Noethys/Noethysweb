...
from core.forms.reset_password import MyPasswordResetForm, MySetPasswordForm
from core.views.login import ClassCommuneLogin
from core.utils import utils_secquest1
...
class MyPasswordResetView(ClassCommuneLogin, auth_views.PasswordResetView):
    ...
    template_name = "core/reset_passwordutil.html"
...
class MyPasswordResetConfirmView(ClassCommuneLogin, auth_views.PasswordResetConfirmView):
    ...
    def form_valid(self, form):
        ...
        if "secquest" in form.cleaned_data:
            if not utils_secquest.Check_secquest(utilisateur=self.user.utilisateur, reponse=form.cleaned_data["secquest"]):
