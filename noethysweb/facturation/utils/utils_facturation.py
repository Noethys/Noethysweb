...
class Facturation:
    ...
    def RechercheAgrement(self, IDactivite, date):
        for agrement in self.listeAgrements:
            if (
                agrement.activite_id == IDactivite
                and date >= agrement.date_debut
                and date <= agrement.date_fin
            ):
                return agrement
        return None
    ...
    def GetListeTriee(self, dict_factures={}):
        ...
        liste_keys = [
            (dict_facture["nomSansCivilite"], idfacture)
            for idfacture, dict_facture in dict_factures.items()
        ]
        liste_keys.sort()
        liste_factures = [
            dict_factures[idfacture] for nom, idfacture in liste_keys
        ]
    ...
    def GetDonnees(
        ...
        IDfamille=None,
        liste_IDindividus=[],
        filtre_prestations=None,
        exclusions_prestations=None,
        impayes_factures=False,
        inclure_cotisations_si_conso=False,
    ):
        ...
        if liste_factures:
            ...
            conditions = (
                (Q(activite__in=liste_activites) | Q(activite=None))
                & Q(date__gte=date_debut_temp)
                & Q(date__lte=date_fin)
                & Q(categorie__in=categories_prestations)
            )
            # Filtre facture
            if mode == "facture":
                conditions &= Q(facture_id=None)
            # Filtre famille
            if IDfamille:
            ...
            if liste_IDindividus:
                if 0 in liste_IDindividus:
                    conditions &= Q(individu_id__in=liste_IDindividus) | Q(
            ...
            if filtre_prestations:
                ...
                for nom_prestation in filtre_prestations.split(";"):
                    conditions_prestation |= Q(
                        label__icontains=nom_prestation.strip()
                    )
        ...
        for prestation in prestations:
            ...
            if (
                prestation.categorie == "consommation"
                and prestation.famille_id
                not in liste_familles_has_prestations_activite
            ):
                liste_familles_has_prestations_activite.append(
                    prestation.famille_id
                )

        # Importation des notes à afficher sur la facture
        dict_notes = {}
        for note in Note.objects.filter(
            afficher_facture=True, famille_id__in=liste_familles
        ):
        ...
        if liste_factures:
            ...
            conditions = (
                (
                    Q(prestation__activite__in=liste_activites)
                    | Q(prestation__activite=None)
                )
                & Q(prestation__date__gte=date_debut_temp)
                & Q(prestation__date__lte=date_fin)
            )
            if mode == "facture":
                conditions &= Q(prestation__facture_id=None)
        logger.debug("Recherche des ventilations des factures...")
        ventilations = Ventilation.objects.select_related(
            "prestation",
            "reglement__payeur",
            "reglement__mode",
            "reglement__emetteur",
        ...
        for ventilation in ventilations:
            ...
            if (
                ventilation.reglement_id
                not in dictReglements[ventilation.famille_id]
            ):
                dictReglements[ventilation.famille_id][
                    ventilation.reglement_id
                ] = {
                    "date": ventilation.reglement.date,
                    "montant": ventilation.reglement.montant,
                    "mode": ventilation.reglement.mode,
                    "emetteur": ventilation.reglement.emetteur,
                    "numero": ventilation.reglement.numero_piece,
                    "payeur": ventilation.reglement.payeur,
                    "ventilation": Decimal(0),
                }
            dictReglements[ventilation.famille_id][ventilation.reglement_id][
                "ventilation"
            ] += ventilation.montant

            dictVentilationPrestations.setdefault(
                ventilation.prestation_id, Decimal(0)
            )
            dictVentilationPrestations[ventilation.prestation_id] += (
                ventilation.montant
            )
        ...
        if not liste_factures:
            ...
            if liste_dates:
                ...
                # Fallback si aucune prestation n'est trouvée
                date_min = date_debut if date_debut else datetime.date.today()
                date_max = date_fin if date_fin else datetime.date.today()
        listeQfdates = Quotient.objects.filter(
            Q(date_fin__gte=date_min) & Q(date_debut__lte=date_max)
        )
        ...
        if not liste_factures:
            conditions &= (
                Q(activite__in=liste_activites) | Q(activite=None)
            ) & Q(date__lte=date_debut)
        if impayes_factures:
            conditions &= Q(facture__isnull=False)
        prestations_reports = Prestation.objects.filter(
            conditions, famille_id__in=liste_familles
        )
        ...
        if not liste_factures:
            conditions = (
                Q(prestation__activite__in=liste_activites)
                | Q(prestation__activite=None)
            ) & Q(prestation__date__lte=date_debut)
        else:
            conditions = Q()
        logger.debug("Recherche de la ventilation des reports...")
        ventilations_reports = (
            Ventilation.objects.values("prestation")
            .filter(conditions, famille_id__in=liste_familles)
            .annotate(total=Sum("montant"))
        )

        dictVentilationReports = {}
        for ventilation in ventilations_reports:
            dictVentilationReports[ventilation["prestation"]] = ventilation[
                "total"
            ]
        ...
        for deduction in deductions:
            ...
            dictDeductions[deduction.prestation_id].append(
                {
                    "IDdeduction": deduction.pk,
                    "date": deduction.date,
                    "montant": deduction.montant,
                    "label": deduction.label,
                    "IDaide": deduction.aide_id,
                }
            )
        ...
        consommations = Consommation.objects.filter(
            conditions, inscription__famille_id__in=liste_familles
        )

        dictConsommations = {}
        for consommation in consommations:
            dictConsommations.setdefault(consommation.prestation_id, [])
            dictConsommations[consommation.prestation_id].append(
                {"date": consommation.date, "etat": consommation.etat}
            )

        # Recherche du solde du compte
        logger.debug(
            "Recherche du total des prestations pour chaque facture..."
        )
        total_prestations = (
            Prestation.objects.values("famille")
            .filter(famille_id__in=liste_familles)
            .annotate(total=Sum("montant"))
        )
        ...
        total_reglements = (
            Reglement.objects.values("famille")
            .filter(famille_id__in=liste_familles)
            .annotate(total=Sum("montant"))
        )
        ...
        for IDfamille_temp in liste_familles:
            ...
            if solde_compte > Decimal(0):
                solde_compte = "+%.2f %s" % (
                    solde_compte,
                    utils_preferences.Get_symbole_monnaie(),
                )
            else:
                solde_compte = "%.2f %s" % (
                    solde_compte,
        ...
        for prestation in prestations:
            ...
            if (
                inclure_cotisations_si_conso
                and prestation.categorie == "cotisation"
                and prestation.famille_id
                not in liste_familles_has_prestations_activite
            ):
                continue
            ...
            if (
                prestation.facture_id
                not in dictComptesPayeursFactures[prestation.famille_id]
            ):
                dictComptesPayeursFactures[prestation.famille_id].append(
                    prestation.facture_id
                )
            ...
            if ID not in dictComptes:
                ...
                rue_resid = (
                    prestation.famille.rue_resid
                    if prestation.famille.rue_resid
                    else ""
                )
                cp_resid = (
                    prestation.famille.cp_resid
                    if prestation.famille.cp_resid
                    else ""
                )
                ville_resid = (
                    prestation.famille.ville_resid
                    if prestation.famille.ville_resid
                    else ""
                )

                # Adresse de facturation
                destinataire_nom = (
                    prestation.famille.facturation_nom
                    or nomsTitulairesAvecCivilite
                )
                destinataire_rue = (
                    prestation.famille.facturation_rue_resid or rue_resid
                )
                destinataire_cp = (
                    prestation.famille.facturation_cp_resid or cp_resid
                )
                destinataire_ville = (
                    prestation.famille.facturation_ville_resid or ville_resid
                )
                ...
                if prestation.famille_id in dict_soldes_comptes:
                    ...
                    solde_compte = (
                        "0.00 %s" % utils_preferences.Get_symbole_monnaie()
                    )
                ...
                dictComptes[ID] = {
                    ...
                    "{TOTAL_REPORTS}": "0.00 %s"
                    % utils_preferences.Get_symbole_monnaie(),
                    "solde_avec_reports": Decimal(0),
                    "{SOLDE_AVEC_REPORTS}": "0.00 %s"
                    % utils_preferences.Get_symbole_monnaie(),
                    "{SOLDE_COMPTE}": solde_compte,
                    "select": True,
                    "messages_familiaux": dict_notes.get(
                        prestation.famille_id, []
                    ),
                    "{NOM_LOT}": "",
                    ...
                    "{DATE_EDITION_LONG}": utils_dates.DateComplete(
                        date_edition
                    ),
                    "{DATE_EDITION_COURT}": utils_dates.ConvertDateToFR(
                        date_edition
                    ),
                ...
                if date_echeance != None:
                    if date_echeance != None:
                        ...
                        dictComptes[ID]["{DATE_ECHEANCE_LONG}"] = (
                            utils_dates.DateComplete(date_echeance)
                        )
                        dictComptes[ID]["{DATE_ECHEANCE_COURT}"] = (
                            utils_dates.ConvertDateToFR(date_echeance)
                        )
                        dictComptes[ID]["{TEXTE_ECHEANCE}"] = (
                            "Echéance du règlement : %s"
                            % utils_dates.ConvertDateToFR(date_echeance)
                        )
                else:
                ...
                for dictReponse in self.questionnaires.GetDonnees(IDfamille):
                    dictComptes[ID][dictReponse["champ"]] = dictReponse[
                        "reponse"
                    ]
                    if dictReponse["controle"] == "codebarres":
                        dictComptes[ID][
                            "{CODEBARRES_QUESTION_%d}"
                            % dictReponse["IDquestion"]
                        ] = dictReponse["reponse"]

            # Insert les montants pour le compte payeur
            if prestation.pk in dictVentilationPrestations:
                montant_ventilation = Decimal(
                    dictVentilationPrestations[prestation.pk]
                )
            else:
            ...
            dictComptes[ID]["solde"] = (
                dictComptes[ID]["total"] - dictComptes[ID]["ventilation"]
            )

            dictComptes[ID]["{TOTAL_PERIODE}"] = "%.02f %s" % (
                dictComptes[ID]["total"],
                utils_preferences.Get_symbole_monnaie(),
            )
            dictComptes[ID]["{TOTAL_REGLE}"] = "%.02f %s" % (
                dictComptes[ID]["ventilation"],
                utils_preferences.Get_symbole_monnaie(),
            )
            dictComptes[ID]["{SOLDE_DU}"] = "%.02f %s" % (
                dictComptes[ID]["solde"],
                utils_preferences.Get_symbole_monnaie(),
            )
            ...
            if (IDindividu in dictComptes[ID]["individus"]) == False:
                if IDindividu:
                    ...
                    if prestation.individu.date_naiss:
                        if prestation.individu.Get_sexe() == "M":
                            texteDateNaiss = (
                                ", né le %s"
                                % utils_dates.ConvertDateToFR(
                                    prestation.individu.date_naiss
                                )
                            )
                        else:
                            texteDateNaiss = (
                    ...
                    texteIndividu = "<b>%s %s</b><font size=6>%s</font>" % (
                        prestation.individu.nom,
                        prestation.individu.prenom or "",
                        texteDateNaiss,
                    )
                ...
                dictComptes[ID]["individus"][IDindividu] = {
                    "texte": texteIndividu,
                    "activites": {},
                    "total": Decimal(0),
                    "ventilation": Decimal(0),
            ...
            if (
                IDactivite
                not in dictComptes[ID]["individus"][IDindividu]["activites"]
            ):
                texteActivite = (
                    prestation.activite.nom if prestation.activite else None
                )
                agrement = self.RechercheAgrement(IDactivite, prestation.date)
                if agrement:
                    texteActivite += " - n° agrément : %s" % agrement
                dictComptes[ID]["individus"][IDindividu]["activites"][
                    IDactivite
            ...
            if (
                ...
                dictComptes[ID]["individus"][IDindividu]["activites"][
                    IDactivite
                ]["presences"][prestation.date] = {
                    "texte": utils_dates.ConvertDateToFR(prestation.date),
                    "unites": [],
                    "total": Decimal(0),
                }
            ...
            dictPrestation = {
                ...
                "categorie": prestation.categorie_tarif,
                "label": label,
                "montant_initial": prestation.montant_initial,
                "montant": prestation.montant,
                "tva": prestation.tva,
                "IDtarif": prestation.tarif_id,
                "nomTarif": prestation.tarif.nom_tarif.nom
                if prestation.tarif
                else None,
                "nomCategorieTarif": prestation.categorie_tarif.nom
                if prestation.categorie_tarif
                else None,
            ...
            dictComptes[ID]["individus"][IDindividu]["activites"][IDactivite][
                "presences"
            ][prestation.date]["unites"].append(dictPrestation)

            # Ajout des totaux
            if prestation.montant:
                dictComptes[ID]["individus"][IDindividu]["total"] += (
                    prestation.montant
                )
                dictComptes[ID]["individus"][IDindividu]["activites"][
                    IDactivite
                ]["presences"][prestation.date]["total"] += prestation.montant
            if montant_ventilation != None:
                dictComptes[ID]["individus"][IDindividu]["ventilation"] += (
                    montant_ventilation
                )
            ...
            for quotient in listeQfdates:
                if (
                    quotient.famille_id == prestation.famille_id
                    and quotient.date_debut <= date_fin
                    and quotient.date_fin >= date_debut
                ):
                    if quotient.date_debut < date_debut:
                        plage = "du %s " % utils_dates.ConvertDateToFR(
                            date_debut
                        )
                    else:
                        plage = "du %s " % utils_dates.ConvertDateToFR(
                    ...
                    if quotient.date_fin > date_fin:
                        plage = plage + "au %s" % utils_dates.ConvertDateToFR(
                            date_fin
                        )
                    else:
                        plage = plage + "au %s" % utils_dates.ConvertDateToFR(
        ...
        for ID, valeurs in dictComptes.items():
            ...
            dictComptes[ID]["{TOTAL_DEDUCTIONS}"] = "%.02f %s" % (
                totalDeductions,
                utils_preferences.Get_symbole_monnaie(),
            )
        ...
        for prestation in prestations_reports:
            montant_ventilation = Decimal(
                dictVentilationReports.get(prestation.pk, 0)
            )
            ...
            if montant_ventilation != prestation.montant:
                if len(liste_factures) == 0:
                    if (
                        prestation.famille_id in dictComptes
                        and prestation.pk
                        not in dictComptes[prestation.famille_id][
                            "listeIDprestations"
                        ]
                    ):
                        if (
                            periode
                            not in dictComptes[prestation.famille_id]["reports"]
                        ):
                            dictComptes[prestation.famille_id]["reports"][
                                periode
                            ] = Decimal(0)
                        dictComptes[prestation.famille_id]["reports"][
                            periode
                        ] += montant_impaye
                        dictComptes[prestation.famille_id]["total_reports"] += (
                            montant_impaye
                        )
                        dictComptes[prestation.famille_id][
                            "{TOTAL_REPORTS}"
                        ] = "%.02f %s" % (
                            dictComptes[prestation.famille_id]["total_reports"],
                            utils_preferences.Get_symbole_monnaie(),
                    ...
                    if prestation.famille_id in dictComptesPayeursFactures:
                        for IDfacture in dictComptesPayeursFactures[
                            prestation.famille_id
                        ]:
                            if (
                                prestation.date
                                < dictComptes[IDfacture]["date_debut"]
                                and prestation.pk
                                not in dictComptes[IDfacture][
                                    "listeIDprestations"
                                ]
                            ):
                                if (
                                    periode
                                    not in dictComptes[IDfacture]["reports"]
                                ):
                                    dictComptes[IDfacture]["reports"][
                                        periode
                                    ] = Decimal(0)
                                dictComptes[IDfacture]["reports"][periode] += (
                                    montant_impaye
                                )
                                dictComptes[IDfacture]["total_reports"] += (
                                    montant_impaye
                                )
                                dictComptes[IDfacture]["{TOTAL_REPORTS}"] = (
                                    "%.02f %s"
        ...
        for ID, dictValeurs in dictComptes.items():
            dictComptes[ID]["solde_avec_reports"] = (
                dictComptes[ID]["solde"] + dictComptes[ID]["total_reports"]
            )
            dictComptes[ID]["{SOLDE_AVEC_REPORTS}"] = "%.02f %s" % (
                dictComptes[ID]["solde_avec_reports"],
                utils_preferences.Get_symbole_monnaie(),
            )
            dictComptes[ID]["{INDIVIDUS_CONCERNES}"] = (
                utils_texte.Convert_liste_to_texte_virgules(
                    dictComptes[ID]["{INDIVIDUS_CONCERNES}"]
                )
            )
            dictComptes[ID]["{NOMS_INDIVIDUS}"] = dictComptes[ID][
                "{INDIVIDUS_CONCERNES}"
            ]
    ...
    def GetDonneesImpression(self, liste_factures=[], dict_options=None):
        ...
        factures = Facture.objects.select_related("lot").filter(
            idfacture__in=liste_factures
        )
        if not factures:
            return False
        ...
        # Prélèvements
        dict_prelevements = {
            prelevement.facture_id: prelevement
            for prelevement in Prelevements.objects.select_related(
                "lot", "lot__modele__compte", "mandat"
            ).filter(facture_id__in=liste_factures)
        }

        # Infos PES ORMC
        dict_pes = {
            piece.facture_id: piece
            for piece in PesPiece.objects.select_related(
                "lot",
                "lot__modele",
                "lot__modele__compte",
                "prelevement_mandat",
        ...
        infosIndividus = utils_infos_individus.Informations(
            liste_familles=liste_idfamille
        )
        ...
        dictComptes = self.GetDonnees(
            ...
            type_label=dict_options["intitules"],
            impayes_factures=dict_options.get("impayes_factures", False),
        )
        ...
        for facture in factures:
            if facture.pk in dictComptes:
                ...
                dictCompte["{DATE_DEBUT}"] = utils_dates.ConvertDateToFR(
                    facture.date_debut
                )
                dictCompte["{DATE_FIN}"] = utils_dates.ConvertDateToFR(
                    facture.date_fin
                )
                dictCompte["{DATE_EDITION_FACTURE}"] = (
                    utils_dates.ConvertDateToFR(facture.date_edition)
                )
                dictCompte["{DATE_ECHEANCE}"] = (
                    utils_dates.ConvertDateToFR(facture.date_echeance)
                    if facture.date_echeance
                    else ""
                )
                dictCompte["{SOLDE}"] = "%.2f %s" % (
                    dictCompte["solde"],
                    utils_preferences.Get_symbole_monnaie(),
                )
                dictCompte["{SOLDE_LETTRES}"] = (
                    utils_conversion.trad(facture.solde).strip().capitalize()
                )
                dictCompte["{SOLDE_AVEC_REPORTS}"] = "%.2f %s" % (
                    dictCompte["solde_avec_reports"],
                    utils_preferences.Get_symbole_monnaie(),
                )
                dictCompte["{SOLDE_AVEC_REPORTS_LETTRES}"] = (
                    utils_conversion.trad(
                        facture.solde + dictCompte["total_reports"]
                    )
                    .strip()
                    .capitalize()
                )
                ...
                for key, valeur in infosIndividus.GetDictValeurs(
                    mode="famille", ID=facture.famille_id, formatChamp=True
                ).items():
                    if key not in (
                        "{FAMILLE_NOM}",
                        "{FAMILLE_RUE}",
                ...
                if prelevement:
                    verbe = (
                        "a été"
                        if prelevement.lot.date < facture.date_edition
                        else "sera"
                    )
                    if prelevement.mandat.iban:
                        dictCompte["prelevement"] = (
                            "La somme de %.2f %s %s prélevée le %s sur le compte ***%s"
                            % (
                                prelevement.montant,
                                utils_preferences.Get_symbole_monnaie(),
                                verbe,
                                utils_dates.ConvertDateToFR(
                                    prelevement.lot.date
                                ),
                                prelevement.mandat.iban[-7:],
                            )
                        )
                    else:
                        dictCompte["prelevement"] = (
                            "La somme de %.2f %s %s prélevée le %s"
                            % (
                                prelevement.montant,
                                utils_preferences.Get_symbole_monnaie(),
                                verbe,
                    ...
                    if prelevement.mandat.rum:
                        dictCompte["prelevement"] += (
                            "<br/>Réf. mandat unique : %s / Code ICS : %s"
                            % (
                                prelevement.mandat.rum,
                                prelevement.lot.modele.compte.code_ics,
                            )
                        )
                    dictCompte["{DATE_PRELEVEMENT}"] = (
                        utils_dates.ConvertDateToFR(prelevement.lot.date)
                    )
                ...
                if piece_pes and piece_pes.prelevement:
                    verbe = (
                        "a été"
                        if piece_pes.lot.date_prelevement < facture.date_edition
                        else "sera"
                    )
                    if piece_pes.prelevement_mandat.iban:
                        dictCompte["prelevement"] = (
                            ...
                            % (
                                ...
                                verbe,
                                utils_dates.ConvertDateToFR(
                                    piece_pes.lot.date_prelevement
                                ),
                                piece_pes.prelevement_mandat.iban[-7:],
                            )
                        )
                    else:
                        dictCompte["prelevement"] = (
                    ...
                    if piece_pes.prelevement_mandat.rum:
                        dictCompte["prelevement"] += (
                            "<br/>Réf. mandat unique : %s / Code ICS : %s"
                            % (
                                piece_pes.prelevement_mandat.rum,
                                piece_pes.lot.modele.compte.code_ics,
                            )
                        )
                    dictCompte["{DATE_PRELEVEMENT}"] = (
                        utils_dates.ConvertDateToFR(
                ...
                dictCompte["{PES_IDLOT}"] = (
                    piece_pes.lot_id if piece_pes else ""
                )
                dictCompte["{PES_NOM_LOT}"] = (
                    piece_pes.lot.nom if piece_pes else ""
                )
                dictCompte["{PES_LOT_EXERCICE}"] = (
                    piece_pes.lot.exercice if piece_pes else ""
                )
                dictCompte["{PES_LOT_MOIS}"] = (
                    piece_pes.lot.mois if piece_pes else ""
                )
                # dictCompte["{PES_LOT_OBJET}"] = dictPes[IDfacture]["pes_lot_objet"]
                dictCompte["{PES_LOT_ID_BORDEREAU}"] = (
                    piece_pes.lot.id_bordereau if piece_pes else ""
                )
                ...
                if piece_pes:
                    try:
                        dictCompte["{PES_DATAMATRIX}"] = Calculer_datamatrix(
                            dictCompte
                        )
                ...
                dictChampsFusion[facture.pk]["{NUMERO_FACTURE}"] = dictCompte[
                    "{NUM_FACTURE}"
                ]
                dictChampsFusion[facture.pk]["{DATE_DEBUT}"] = (
                    utils_dates.ConvertDateToFR(facture.date_debut)
                )
                dictChampsFusion[facture.pk]["{DATE_FIN}"] = (
                    utils_dates.ConvertDateToFR(facture.date_fin)
                )
                dictChampsFusion[facture.pk]["{DATE_EDITION_FACTURE}"] = (
                    utils_dates.ConvertDateToFR(facture.date_edition)
                )
                dictChampsFusion[facture.pk]["{DATE_ECHEANCE}"] = (
                    utils_dates.ConvertDateToFR(facture.date_echeance)
                    if facture.date_echeance
                    else ""
                )
                dictChampsFusion[facture.pk]["{SOLDE}"] = "%.2f %s" % (
                    dictCompte["solde"],
                    utils_preferences.Get_symbole_monnaie(),
                )
                dictChampsFusion[facture.pk]["{SOLDE_AVEC_REPORTS}"] = (
                    dictCompte["{SOLDE_AVEC_REPORTS}"]
                )
                dictChampsFusion[facture.pk]["{SOLDE_COMPTE}"] = dictCompte[
                    "{SOLDE_COMPTE}"
                ]
                dictChampsFusion[facture.pk]["{DATE_PRELEVEMENT}"] = dictCompte[
                    "{DATE_PRELEVEMENT}"
                ]

                # Fusion pour textes personnalisés
                dictCompte["texte_titre"] = self.RemplaceMotsCles(
                    dict_options["texte_titre"], dictCompte
                )
                dictCompte["texte_introduction"] = self.RemplaceMotsCles(
                    dict_options["texte_introduction"], dictCompte
                )
                dictCompte["texte_conclusion"] = self.RemplaceMotsCles(
                    dict_options["texte_conclusion"], dictCompte
                )
    ...
    def Impression(
        self, liste_factures=[], dict_options=None, mode_email=False
    ):
        """Impression des factures"""
        ...
        if mode_email:
            ...
            for IDfacture, dictFacture in dictFactures.items():
                logger.debug(
                    "Création du PDF de la facture ID%d..." % IDfacture
                )
                impression.Generation_document(
                    dict_donnees={IDfacture: dictFacture}
                )
                noms_fichiers[IDfacture] = {
                    "nom_fichier": impression.Get_nom_fichier(),
                    "valeurs": impression.Get_champs_fusion_pour_email(
                        "facture", IDfacture
                    ),
                }
...
def Calculer_datamatrix(dictCompte):
    ...
    elements.append(
        "%03d"
        % int(
            data_codes_etab.Rechercher(
                str(piece_pes.lot.modele.code_prodloc[:2])
            )
        )
    )
    ...
    cle = (
        sum([rang * int(valeur) for rang, valeur in enumerate(base[::-1], 1)])
        % 100
    )
    ...
    cle2 = Get_cle_modulo_23(
        (
            str(piece_pes.lot.exercice)[-2:],
            str(piece_pes.lot.mois),
            "00",
            str(dictCompte["facture"].numero).zfill(13),
        )
    )
    ...
    somme1 = sum(
        [rang * int(valeur) for rang, valeur in enumerate(base1[::-1], 9)]
    )

    base2 = "".join(elements[-1]).replace(" ", "0")
    somme2 = sum(
        [rang * int(valeur) for rang, valeur in enumerate(base2[::-1], 1)]
    )
