...
class Formulaire(FormulaireBase, ModelForm):
    email_depots = forms.BooleanField(label="Activer l'envoi des avis d'encaissement par Email", required=False, initial=False)
    ...
    class Meta:
        model = Famille
        fields = ["email_depots", "email_depots_adresses"]

    def __init__(self, *args, **kwargs):
        idfamille = kwargs.pop("idfamille")
        super(Formulaire, self).__init__(*args, **kwargs)
        self.helper = FormHelper()
        self.helper.form_id = 'famille_abo_depots_form'
    ...
    def clean(self):
        if self.cleaned_data.get("email_depots"):
            ...
            self.cleaned_data["email_depots_adresses"] = "##".join(liste_adresses)
        else:
            # Si non activé
            self.cleaned_data["email_depots_adresses"] = None
