...
class Formulaire(FormulaireBase, ModelForm):
    ...
    class Meta:
        model = Famille
        fields = ["email_factures", "email_factures_adresses"]

    def __init__(self, *args, **kwargs):
        idfamille = kwargs.pop("idfamille")
        super(Formulaire, self).__init__(*args, **kwargs)
        self.helper = FormHelper()
        self.helper.form_id = 'famille_abo_factures_form'
        ...
        self.helper.layout = Layout(
            Commandes(annuler_url="{% url 'famille_factures_liste' idfamille=idfamille %}", ajouter=False),
            Fieldset("Activation du service",
                Field("email_factures"),
            ),
    ...
    def clean(self):
        if self.cleaned_data.get("email_factures"):
            ...
            self.cleaned_data["email_factures_adresses"] = "##".join(liste_adresses)
        else:
            # Si non activé
