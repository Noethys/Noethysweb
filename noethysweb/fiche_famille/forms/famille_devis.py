...
class Formulaire(ModelForm):
    ...
    modele = forms.ModelChoiceField(label="Modèle de document", widget=Select2Widget({"lang": "fr"}), queryset=ModeleDocument.objects.filter(categorie="devis").order_by("nom"), required=True)
