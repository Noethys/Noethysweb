...
from parametrage.forms.activites_groupes import Formulaire
...
class Page(Onglet):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['box_titre'] = "Groupes"
        context['onglet_actif'] = "groupes"
...
class Liste(Page, crud.Liste):
    ...
    def get_queryset(self):
        return Groupe.objects.filter(Q(activite=self.Get_idactivite()) & self.Get_filtres("Q"))
    ...
    class datatable_class(MyDatatable):
        filtres = ['idgroupe', 'nom', 'abrege']

        actions = columns.TextColumn("Actions", sources=None, processor='Get_actions_speciales')

        class Meta:
            structure_template = MyDatatable.structure_template
            columns = ['idgroupe', 'ordre', 'nom', 'abrege']
            #hidden_columns = = ["idgroupe"]
            ordering = ['ordre']
...
class Supprimer(Page, crud.Supprimer):
    ...
    #         liste_objects = self.model.objects.filter(activite=self.get_object().activite).order_by("ordre")
    #         ordre = 1
