        ...
        if Assurance.objects.exclude(individu_id=self.idindividu).filter(famille_id=self.idfamille):
            html = """
                <div class="pull-right">
                    ...
                    <a class="btn btn-danger" href="{{% url 'portail_individu_assurances' idrattachement=idrattachement %}}" title="{title}"><i class="fa fa-ban margin-r-5" aria-hidden="true"></i>{label2}</a>
                </div>
            """.format(label1=_("Importer les assurances cochées"), title=_("Annuler"), label2=_("Annuler"))
        else:
            html = """
                <div class="pull-right">
                    <a class="btn btn-danger" href="{{% url 'portail_individu_assurances' idrattachement=idrattachement %}}" title="{title}"><i class="fa fa-ban margin-r-5" aria-hidden="true"></i>{label}</a>
                </div>
