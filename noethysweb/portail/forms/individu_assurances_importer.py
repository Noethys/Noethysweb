...
class Formulaire(FormulaireBase, forms.Form):
    assurances = forms.CharField(label="Assurances disponibles", required=False, widget=SelectionAssurancesAutresFiches(
        attrs={"texte_si_vide": "Aucune assurance disponible", "hauteur_libre": True, "coche_tout": False}))
        ...
        if Assurance.objects.exclude(individu_id=self.idindividu).filter(famille_id=self.idfamille):
            html = """
                <div class="pull-right">
                    <button type="submit" class='btn btn-primary' name="enregistrer"><i class="fa fa-check margin-r-5"></i>Importer les assurances cochées</button> 
                    <a class="btn btn-danger" href="{% url 'portail_individu_assurances' idrattachement=idrattachement %}" title="Annuler"><i class="fa fa-ban margin-r-5"></i>Annuler</a>
                </div>
            """
        else:
            html = """
